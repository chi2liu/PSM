		...
		if (findstr != null || findstr.length() > 0) {
			...
			for (int i = 0; i < strKey.length; i++) {
				if (strKey[i].length() > 0) {
					hql += " and ( jobActivity like '%" + strKey[i] + "%' or badFactor like '%" + strKey[i] + strKey[i]
							+ "%' or rank like '%" + strKey[i] + "%' or controlAction like '%"
							+ "%' or badEvent like '%" + strKey[i] + "%' or yijian like '%" + strKey[i] + "%' )";
				}
			}
		}
		Query query = getSession().createQuery(hql);
