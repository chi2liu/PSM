Ext.define('MapGrid', {
    ...
    createGrid: function (config) {  
 		        			...
 		        			forms.form.submit({
 		        				...
 		        				url:encodeURI(url),
 		  	                	success: function(form, action){	  	                	
 		  	                		store.getProxy().url = 'MapAction!getMonitorListInPage';
    				...
    				if (buttonID === 'yes'){			
    					$.getJSON(encodeURI('MapAction!deleteMonitor'),
                                ...
                                function (res) 
                                {
    								if (res.success) 
    								{
    									store.getProxy().url = encodeURI('MapAction!getMonitorListInPage');
    									store.load({ params: { start: 0, limit: psize } });
		...
		var store_Monitor = Ext.create('Ext.data.Store', {  //监控列表store
            ...
            proxy: {
                type: 'ajax',
                url: encodeURI('MapAction!getMonitorListInPage'),
		...
		gridDT = Ext.create('Ext.panel.Panel', {               //百度地图Grid
			...
			column = [
	            	    ...
	            	    { text: '端口号', dataIndex: 'port', align: 'center', width: 60},
