				...
				Ext.Msg.confirm('删除', '确定彻底删除吗？', function (buttonID) {
					if (buttonID === 'yes') {
						...
						if (title == '安全理念' || title == '安全警示语' || title == '安全承诺' || title == '安全行为激励'
							|| tableID >= 307 && tableID <= 311) delete_url = 'EduTrainAction!deleteMultimediafile';
                		...
                		listeners:
						{
							'click': function (item, e, eOpts) {
                        		...
                        		Ext.Msg.confirm('删除', '确定彻底删除吗？', function (buttonID) {
				    				...
				    				{
				    					//alert(item.text);
				    					$.getJSON(encodeURI("GoalDutyAction!deleteFileUpload?file=" + item.text + "&title=" + title),
				    							//{id: keyIDs.toString()},	//Ajax参数
				                                function (res) 
				                                {
				    								if (res.success) 
...
//jianglf-----------------------------------------------------------------------------------------------------     
        if (title == '安全理念' || title == '安全警示语' || title == '安全承诺' || title == '安全行为激励'
			...
			tbar.add(btnAdd);
			tbar.add(btnDel);
			if(user.role === '项目部人员' && !(tableID >= 307 && tableID <= 311)) {
				tbar.remove(btnAdd);
				tbar.remove(btnDel);
    		}
